...
const CoursePage = () => {
  ...
  // ✅ Wrap fetchCourses with useCallback to satisfy ESLint
  ...
  useEffect(() => {
    fetchCourses();
  }, [fetchCourses]); // Add fetchCourses as dependency
  ...
  return (
    <div className="course-page">
      ...
      <style>{`
        ...
        .page-title { font-size: 2.6rem; font-weight: 700; color: #1e3a8a; text-align: center; margin-bottom: 40px; animation: fadeDown 1s ease; }
        @keyframes fadeDown { from { opacity: 0; transform: translateY(-20px); } to { opacity: 1; transform: translateY(0); } }
        .form-container { background: #ffffff; padding: 30px; border-radius: 12px; box-shadow: 0 5px 20px rgba(0,0,0,0.1); max-width: 600px; margin: 0 auto 50px; }
        .form-title { font-size: 1.5rem; margin-bottom: 20px; color: #1e40af; }
        .course-form input, .course-form textarea { width: 100%; padding: 12px; margin-bottom: 12px; border-radius: 8px; border: 1px solid #cbd5e1; transition: 0.3s; }
        .course-form input:focus, .course-form textarea:focus { outline: none; border-color: #2563eb; box-shadow: 0 0 0 2px #bfdbfe; }
        .btn { padding: 10px 18px; border: none; border-radius: 8px; cursor: pointer; font-weight: 500; transition: all 0.3s ease; }
        .btn.primary { background: linear-gradient(90deg, #2563eb, #1e40af); color: #fff; } .btn.primary:hover { transform: scale(1.05); }
        .btn.green { background: #16a34a; color: white; } .btn.red { background: #dc2626; color: white; }
        .course-list { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 25px; padding: 0 20px; }
        .course-card { background: #ffffff; border-radius: 14px; padding: 20px; box-shadow: 0 8px 20px rgba(0,0,0,0.08); transition: transform 0.3s ease, box-shadow 0.3s ease; animation: fadeUp 0.6s ease forwards; }
        .course-card:hover { transform: translateY(-6px); box-shadow: 0 10px 25px rgba(0,0,0,0.15); }
        @keyframes fadeUp { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }
        .course-title { color: #1e3a8a; text-decoration: none; font-size: 1.2rem; font-weight: 600; }
        .course-desc { color: #374151; font-size: 0.95rem; margin: 10px 0; min-height: 50px; }
        .action-buttons, .student-action { margin-top: 15px; }
